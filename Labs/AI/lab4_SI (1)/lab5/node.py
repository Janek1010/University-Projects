...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for possible_split in possible_splits:
            left_labels = y[:possible_split+1]
            right_labels = y[possible_split+1:]

            left_positive = np.sum(left_labels == 1)
            left_negative = np.sum(left_labels == 0)
            right_positive = np.sum(right_labels == 1)
            right_negative = np.sum(right_labels == 0)

            left = left_negative + left_positive
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        indices = np.random.choice(X.shape[1], feature_subset)
